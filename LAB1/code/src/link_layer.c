...
int sendSet()
{
    ...
    while (state != STOP && alarmCounter < (llObject.nRetransmissions + 1))
    {
        if (!alarmFlag)
        {
            printf("Set Written\n");
...
void alarmController(int signal) {
    alarmFlag = FALSE;   // alarm flag -> TRUE when the alarm has occured
    alarmCounter++;
    printf(" alarme\n"); 
}
...
int llwrite(int fd, const unsigned char *buf, int bufSize)
{
   unsigned char trama_info[2*MAX_PAYLOAD_SIZE];
    ...
    (void)signal(SIGALRM,alarmController);    
    while(status_llwrite != STOP && alarmCounter < (llObject.nRetransmissions + 1)) 
    {
        //printf("The value of the alarmFlag %d\n", alarmFlag);
        //printf("status_llwrite == REJ%d\n",status_llwrite == REJ); definition of the link-connection
...
int llread(unsigned char *packet) {
    ...
    while (1) {
        if (read(fd, buf + sz, 1) > 0) {
            ...
            stateMachine(&status_llread, buf[sz], 1); // Process the received data.
            sz++;
...
sendDiscCommand()
{
    ...
    buf[4] = FLAG;
    (void)signal(SIGALRM,alarmController);
    while(status_llclose != STOP && alarmCounter < (llObject.nRetransmissions + 1))
    {
        if(!alarmFlag)
        {
            printf("Disc Written\n");
